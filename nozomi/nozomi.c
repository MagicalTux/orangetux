 ...
 *
 * Version 2.1c
 * 01-October-2007 Mark Karpeles
 * - DTR toggling patch by Eric Lammerts
 * - Commented TIOCMBIS/TIOCMBIC ioctls (bogus), confirmed by Alan Cox
 * - devexit patch by Satyam Sharma
 * - patch for PLUGDEV=n by Randy Dunlap
 *
 * Version 2.1b
 ...
 * -Rewrite cleaner
 */
static void read_mem32(u32 *buf, const void __iomem *mem_addr_start, u32 size_bytes)
{
	u32 i = 0;
#ifdef __ARMEB__
...
static int nozomi_read_config_table(struct nozomi *dc)
{
	...
	/* DBG1( "0x%08X == 0x%08X ", dc->config_table.signature, CONFIG_MAGIC); */
	...
	if ((dc->config_table.version > 0)
		...
		write_mem32(dc->port[PORT_MDM].ul_addr[CH_A], (u32*) &offset, 4);
		write_mem32(dc->port[PORT_MDM].ul_addr[CH_B], (u32*) &offset, 4);
...
static int receive_data(enum port_type index, struct nozomi *dc)
{
	...
	if (test_bit(TTY_THROTTLED, &tty->flags)) {
		DBG1("No room in tty, don't read data, don't ack interrupt, disable interrupt");
	...
	while (size > 0) {
		...
		if (size < RECEIVE_BUF_MAX) {
			...
			while (i < RECEIVE_BUF_MAX) {
				tty_insert_flip_char(tty, buf[i], TTY_NORMAL);
				i++;
			}
			size -= RECEIVE_BUF_MAX;
			offset += RECEIVE_BUF_MAX;
		}
	}

	port->rx_data += received;
...
static int receive_flow_control(struct nozomi *dc, struct irq *m)
{
	...
	switch (ctrl_dl.port) {
	case CTRL_CMD:
		DBG1("The Base Band sends this value as a response to a request for IMSI detach sent" " over the control channel uplink (see section 7.6.1).");
		break;
	case CTRL_MDM:
		port = PORT_MDM;
		enable_ier = MDM_DL;
	...
	if (old_ctrl.CTS == 1 && ctrl_dl.CTS == 0) {
		DBG1("Disable interrupt (0x%04X) on port: %d", enable_ier, port);
		disable_transmit_ul(port, dc);

	} else if (old_ctrl.CTS == 0 && ctrl_dl.CTS == 1) {

		if (__kfifo_len(dc->port[port].fifo_ul)) {
			DBG1("Enable interrupt (0x%04X) on port: %d", enable_ier,
			   port);
...
static enum ctrl_port_type port2ctrl(enum port_type port, const struct nozomi *dc)
{
	switch (port) {
	case PORT_MDM:
		return CTRL_MDM;
...
static int send_flow_control(struct nozomi *dc)
{
	...
	for (i = PORT_MDM; i < MAX_PORT; i++) {
		if (dc->port[i].update_flow_control) {
			...
			write_mem32(dc->port[PORT_CTRL].ul_addr[0], (u32*) ctrl,
				2);
...
static irqreturn_t interrupt_handler(int irq, void *dev_id)
{
	...
	/* Just handle interrupt enabled in IER (by masking with dc->last_ier) */
...
static int ntty_open(struct tty_struct *tty, struct file *file)
{
	...
	if (port->tty_open_count == 1) {
		...
		spin_lock_irqsave(&dc->spin_mutex, flags);
		dc->last_ier = (dc->last_ier & ~(port->token_dl)) | port->token_dl;
		writew(dc->last_ier, dc->reg_ier);
		spin_unlock_irqrestore(&dc->spin_mutex, flags);
	}
...
static int ntty_ioctl(struct tty_struct *tty, struct file *file,
		      unsigned int cmd, unsigned long arg)
{
/* - Code reported to be useless/bogus
	struct port *port = tty->driver_data;
	int mask;
*/
	struct nozomi *dc = get_dc_by_tty(tty);
